...
exports.checkUser = function(req, res, next) {
  if (!exports.isLoggedIn(req)) {
    ...
    res.send(
      '<script>alert("Sorry DR you are not logged in please login first to can access this page")</script>' +
        '<script>window.location.href= window.location.origin+"/login" </script>'
    );
  } else {
    console.log('next');
    next();
...
exports.createSession = function(req, res, aUser, username, b) {
  req.session.regenerate(function() {
    ...
    if (b) {
      res.send(
        `Welcome DR.${username} you create new user and you are logged in now`
      );
    } else {
