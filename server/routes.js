  ...
  .post(function(req, res) {
    ...
    User.findOne({ userName: userName }, function(err, user) {
      if (!user) {
        ...
        res.send(
          `Sorry DR. this username does not exist in database please create new user now // if you have account but insert wrong username please go to login page again and insert your correct username`
        );
      } else {
        bcrypt.compare(password, user.password, function(err, match) {
          if (match) {
            ...
            res.send(
              `Sorry DR.${userName} this password is wrong please insert the username again and your correct password`
            );
          }
        });
      }
  ...
  .post(function(req, res) {
    ...
    User.findOne({ userName: userName }, function(err, user) {
      if (!user) {
        ...
        res.send(
          `Sorry DR.${userName} you signup before please insert the username again and your password to log in // if you are not DR.${userName} please go to sign up page again and insert another username`
        );
      }
    });
  });
...
router.route('/newpatient').get(utils.checkUser, function(req, res) {
  res.sendFile(path.join(__dirname, '../react-client/dist/index.html'));
});
